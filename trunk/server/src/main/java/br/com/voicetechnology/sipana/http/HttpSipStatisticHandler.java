...
public class HttpSipStatisticHandler extends AbstractHandler {
    ...
    public HttpSipStatisticHandler(StatisticSip statistics) {
        ...
        String str_refresh = SipanaCapturer.getProperties().getProperty(
                "sipana.http.refresh");
    ...
    public String getHtmlStatistics() {
        ...
        res_html.append("  <META HTTP-EQUIV=\"Refresh\" CONTENT=\""
                + html_refresh + "\">\n");
        res_html
        ...
        res_html.append("  <li>Target IP: " + statistics.getTargetIpAddr()
                + "</li>\n</ul>");
        res_html.append("<h3>Transactions Summary</h3><ul>\n");
        res_html.append("  <li>Completed: " + statistics.getRequestCounter()
                + "</li>\n");
        res_html.append("  <li>Pending: "
                + statistics.getPendingTransactionCounter() + "</li>\n");
        res_html.append("  <li>Timeout: " + statistics.getTimeoutCounter()
                + "</li>\n</ul>\n");
        ...
        for (String method : statistics.getIncomingRequestMethods()) {
            StatisticSipRequest request = statistics
                    .getIncomingRequestStatistic(method);
            int response_codes_size = request.getResponseCodes().size();

            res_html.append("<tr>\n<th rowspan=" + response_codes_size + ">"
                    + method + " (" + request.getCounter() + "/"
                    + request.getTimeoutCounter() + ")\n");
            int line = 1;

            for (int code : request.getResponseCodes()) {
                StatisticSipResponse response = request
                        .getResponseStatistic(code);
        ...
        for (String method : statistics.getOutgoingRequestMethods()) {
            StatisticSipRequest request = statistics
                    .getOutgoingRequestStatistic(method);
            int response_codes_size = request.getResponseCodes().size();
    ...
    public String getXmlStatistics() {
        ...
        res_xml.append(" <targetip>" + statistics.getTargetIpAddr()
                + "</targetip>");
        res_xml.append(" <completed>" + statistics.getRequestCounter()
                + "</completed>\n");
        res_xml.append(" <pending>" + statistics.getPendingTransactionCounter()
                + "</pending>\n");
        res_xml.append(" <timeout>" + statistics.getTimeoutCounter()
                + "</timeout>\n\n");
        ...
        for (String method : statistics.getIncomingRequestMethods()) {
            ...
            res_xml.append("   <timeout>" + request.getTimeoutCounter()
                    + "</timeout>\n");
            ...
            for (int code : request.getResponseCodes()) {
                ...
                res_xml.append("     <status-code>" + response.getStatusCode()
                        + "</status-code>\n");
                res_xml.append("     <min>" + response.getMinTime()
                        + "</min>\n");
                res_xml.append("     <avg>" + response.getSampleAvgTime()
                        + "</avg>\n");
                res_xml.append("     <max>" + response.getMaxTime()
                        + "</max>\n");
                res_xml.append("    </response>\n");
            }
