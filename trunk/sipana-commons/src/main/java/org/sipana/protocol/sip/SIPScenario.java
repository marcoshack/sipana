...
public class SIPScenario {
	...
	private static Logger logger = Logger.getLogger(SIPScenario.class);
	 ...
	 * @return String JPEG image file name. The filename is composed by the 
	 *         prefix "sip_callflow-" and a random generated UUID. 
	 * @author mhack
	 */
	public static String createSIPScenario(List<SIPMessage> messages, String filepath) {
		...
		sbFilePath.append(File.separator).append(sbFilename);
		
		try {
			 
			FileOutputStream file = new FileOutputStream(sbFilePath.toString());
			...
			logger.error("Error creating SIP scenario diagram", e);
			return null;
		}
	}
