...
public class StatisticSip {
    ...
    private StatisticSip() {
        ...
        targetIp = SipanaCapturer.getProperties().getProperty(
                "sipana.capturer.targetIp");
    }
    ...
    public void addTransaction(SipTransaction transaction) {
        ...
        synchronized (requests) {
            ...
            StatisticSipRequest requestStat = requests.get(method);

            if (requestStat == null) {
                requestStat = new StatisticSipRequest(transaction);
                requests.put(method, requestStat);
                logger.debug("Statistics for " + method + " transaction added");
            }

            requestStat.addTransaction(transaction);
            request_counter++;
        }
    }

    private LinkedHashMap<String, StatisticSipRequest> getRequests(
