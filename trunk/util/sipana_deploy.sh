...
# server
cp sipana-server-ejb/target/sipana-server-ejb-1.0.jar $JBOSS_HOME/server/default/deploy/ 
cp sipana-server-war/target/sipana-server-war-1.0.war $JBOSS_HOME/server/default/deploy/ 
cp sipana-server-ear/src/main/resources/*.xml $JBOSS_HOME/server/default/deploy/
